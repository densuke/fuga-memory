...
Gemini のコンテキスト（現セッション内の会話）と fuga-memory（セッションをまたぐ長期記憶）は役割が異なります。
以下のように言葉を使い分けると、意図どおりに機能します。

| 言いたいこと | 自然な言い方の例 | 動作 |
|---|---|---|
| 過去のセッションを思い出してほしい | 「前回〜」「以前やった〜」「過去に話した〜」 | `search_memory` で検索 |
| 重要なことを長期記録したい | 「長期記憶に残して」「重要なので記録して」 | `save_memory` で保存 |
| 現セッション内の話 | 普通に話す | コンテキストをそのまま使う |

> **ヒント**: Gemini CLI には Claude Code のような Stop フック（自動保存）がないため、
> セッション終了時に重要な内容を明示的に `save_memory` で保存することをお勧めします。
> 例: 「今日の作業内容を fuga-memory に保存して」

`~/.gemini/settings.json` の `systemPrompt` にルールを書いておくと、
毎回指示しなくても Gemini が自動で使い分けるようになります：

```json
{
  "systemPrompt": "ユーザーが「前回」「以前」「過去に」という言葉を使ったときは、fuga-memory の search_memory ツールで過去の記憶を検索してから回答してください。「長期記憶に残して」と言われたときは save_memory で保存してください。"
}
```

## 5. Gemini CLI ユーザーへのヒント
