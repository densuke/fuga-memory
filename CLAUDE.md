...
fuga-memory（長期記憶）と Claude のコンテキスト（短期記憶）は目的が異なる。
CLAUDE.md にこのルールを書いておくと、Claude が文脈から判断して適切なほうを使うようになる。
...
- セッション終了時は Stop フックが自動で fuga-memory に保存するため、
  ユーザーは通常「記憶して」と言う必要はない
- 「前回」「以前」などが出たら、まず search_memory を呼んでから回答する
```

### ポイント

- **保存はほぼ自動**: Stop フックがセッション終了時に保存するため、明示的な保存指示は不要
- **取り出しは時間軸で判断**: 「前回／以前／過去に」という言葉が fuga-memory 検索のトリガー
- **「fuga-memoryで」とは言わなくていい**: 上記トリガーワードで自然に使い分けられる
